...
## 5b. batch size를 키우면 빨라지는가

셀마다 정반대다. 같은 셀을 batch만 바꿔 1 epoch씩 실측:

| 셀 | GPU util | batch | s/epoch | GPU reserved |
|---|---:|---:|---:|---:|
| ETTm1 / 96 | 15 % | 32 | 7.9 | 0.03 GB |
| | | 64 | 3.7 | 0.05 GB |
| | | 128 | 2.3 | 0.07 GB |
| | | 256 | **1.4 (x5.6)** | 0.09 GB |
| ECL / 96 | 86 % | 16 | 28.6 | 0.92 GB |
| | | 32 | 27.2 | 1.73 GB |
| | | 64 | **26.8 (x1.07)** | 3.28 GB |
| Traffic / 96 | 94 % | 16 | 93.8 | 5.71 GB |
| | | 32 | **92.0 (x1.02)** | **10.56 GB** |

ETT 계열은 GPU가 15 %만 도는 step-오버헤드 바운드라 배치를 8배 키우면 5.6배
빨라지고 메모리는 거의 그대로다. ECL과 Traffic은 이미 연산 포화라 배치를
2–4배 키워도 2–7 %밖에 안 빨라지면서 메모리만 1.9–3.6배가 된다. Traffic을
batch 32로 올리면 10.56 GB — 12 GB 카드를 거의 채우고 얻는 것은 2 %다.

전체 표에 적용하면 1.9 시간 → 약 1.5 시간(약 20 % 절감)에 그친다. Traffic과
ECL이 전체의 71 %인데 둘 다 배치에 반응하지 않기 때문이다. 배치를 키우면
epoch당 optimizer step이 줄어 early stopping이 늦게 걸리는 경향도 있어 실제
절감은 이보다 작을 수 있다.

**batch size는 공식 프로토콜의 일부**(대부분 32, ECL/Traffic만 16)이므로,
바꾸면 1절의 재현 판정은 더 이상 적용되지 않는다. 재현이 목적이면 건드리지
말고, 로컬에서 빠른 반복이 필요할 때만 ETT 계열에 `--batch-size 256`을 쓰는
것이 실익이 있다(ETTm 8셀 520 s → 112 s).

### gradient accumulation

exp007에는 없다(`--batch-size`만 있다). 그리고 accumulation은 **시간을 줄이는
도구가 아니다** — 같은 forward/backward를 쪼개 수행하므로 연산량은 그대로이고
optimizer step만 줄어 오히려 근소하게 느려진다. 쓸 이유는 하나뿐이다: VRAM이
모자랄 때 effective batch를 유지하는 것. 위 표대로 Traffic이 5.71 GB이므로
8 GB 이상 카드에서는 필요 없고, 필요해지는 경우는 8 GB 미만 카드이거나
`--coe-backprop all`(5절)을 쓸 때다.
